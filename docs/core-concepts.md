...
| 信号 | 方向 | 说明 |
|------|------|------|
| `onReady(signal)` | gitmesh → 调用方 → Agent | 工作区就绪，Agent 可以开始编码 |
| `onConflict(conflict)` | gitmesh → 调用方 → Agent | 发生冲突，需要 Agent 解决 |

Agent 可以是任何东西 — Claude SDK、shell 脚本、HTTP 服务、甚至手工操作。gitmesh 只关心「干完了吗」和「冲突怎么解决」。

调用方的职责：
1. 在 `onReady` 中启动 Agent，Agent 完成后调用 `signal.done()`
2. 在 `onConflict` 中桥接 Agent 解决冲突，返回 `{ resolved: true/false }`

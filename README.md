...
interface AgentDefinition {
  ...
  onReady: (signal: AgentWorkDoneSignal) => void | Promise<void>;// fire-and-forget
  onConflict?: (info: ConflictInfo) => Promise<ResolutionResult>;// 冲突解决回调（完全自定义，优先级最高）
  resolveConflict?: (params: ConflictResolutionParams) => Promise<void>; // 内建循环模式
  conflictPromptOptions?: ConflictPromptOptions;                 // resolveConflict prompt 自定义
}
```

...
对标 treefork 的核心能力但不暴露为公开 API。仅供 session 内部调用。
...
worktree CRUD 对 gitmesh 的调用方没有独立价值。调用方不需要「给我一个 worktree 让我自己管理」——这正好是 treefork 的职责。gitmesh 的 worktree 层只服务于自身的合并编排，暴露出去会增加 API 表面积和维护负担。

如果需要独立的 worktree 管理能力，直接使用 treefork。
...
| **Remote 模式** | 支持从远端 clone 仓库，但合并仍在本地（和 treefork remote 模式同理） |
| **Checkpoint** | 在 rebase 前自动拍 checkpoint，方便回滚（可复用 treefork 的 checkpoint 机制） |
...
checkpoint            冲突路由
config 管理           Agent 协议
CLI + tmux            Session 生命周期
                      事件系统
```

两者互补，不重复。gitmesh 内部的 worktree 层是自包含的轻量实现，不依赖 treefork。如果调用方已经使用 treefork 管理 worktree，gitmesh 未来可以支持通过配置切换到 treefork 作为 worktree 后端。
